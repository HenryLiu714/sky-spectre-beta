...
# Graphics
background = pg.image.load(asset_directory + "board1.jpg").convert()
...
def getFEN():
    ...
    if enp_target == -1: rString += '-'
    else: rString += posToSpace(enp_target)

    rString += ' '
    rString += str(halfmove)

    rString += ' '
    rString += str(fullmove)
    ...
    for set in bsets:
        ...
        while 0<=i<64 and 0<=f<8 and 0<=r<8:
            ...
            if board[i] == 0:
                ...
                moves.append(i)
                if board[i]%8 == 6:
                    if team == WHITE and 0<=i+mset<64:
                        white_threats.append(i+mset)
                    elif team == BLACK and 0<=i+mset<64:
                        black_threats.append(i+mset)
...
def checkLegality(pos, move, incheck=0):
    ...
    if incheck == 1:
        ...
        if board[pos]%8 == 6:
            ...
            if len(check_pieces) > 1 or len(check_pieces) == 0: return 0
...
# position = '4k2r/pp2bpp1/8/1pnqp2n/2bP4/6KP/PBr5/RN3BR1 w k g0 0 4'
...
def UCItoMove(ucimove):
    ...
    rank, file = posToRF(move)

    if board[pos] == 1 and rank == 7:
        promo = ucimove[4]
        board[pos] = PIECE_TYPES[promo]
    elif board[pos]== 9 and rank == 0:
        promo = ucimove[4]
        board[pos] = PIECE_TYPES[promo] + 8
...
while 1:
    ...
    if turn == WHITE and tr == 1:
        space, move = singleBestMove(pieces, turn, board)
        ...
        uci = stockfish.get_best_move(10000)
        pos, move = UCItoMove(uci)
        performMove(pos, move)
        turnswap = 1
    ...
    for space in black_threats:
        displayHighlight(space)
