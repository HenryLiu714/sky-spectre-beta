...
def getMove(fen):
    ...
    current_score = evaluatePosition(turn, board)
    move, score = findMove(3, current_score, turn, board, -1000000)

    return move[1], move[0], board

best_score = -1000000
flag = False
def findMove(depth, current_score, team, board, alpha):
    global best_score
    if depth == 1:
        best_move = [0, 0]
        ...
        if team == turn:
            score = -1000000
            allMoves = getAllMoves(team, board)

            for move in allMoves:
                test_score = testMove(move[0], move[1], board)
                if test_score > score:
                    net_score = current_score + test_score
                    score = test_score
                    best_move = move
    ...
    for move in allMoves:
        ...
        simboard = performMove(move[0], move[1], simboard)

        base_score = evaluatePosition(team, simboard)
        
        if team:
            m, score = findMove(depth - 1, base_score, not team, simboard, alpha)
        else:
            m, score = findMove(depth - 1, base_score, not team, simboard, alpha)

        net_score = base_score + score
