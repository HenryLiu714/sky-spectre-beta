...
def getMove(fen):
    ...
    for space in pieces:
        piece = board[space]
        if piece.team == turn:
            legal_moves = getLegalMoves(piece.type, space, board)

            for move in legal_moves:
                simboard = [x for x in board]
                simboard = performMove(space, move, simboard)

                base_score = evaluatePosition(turn, simboard)
                new_pieces = getPieces(simboard)

                new_score = 100000
                for new_space in new_pieces: # For piece 2
                    flag = False
                    new_piece = simboard[new_space]
                    if simboard[new_space].team != turn:
                        new_moves = getLegalMoves(new_piece.type, new_space, simboard)

                        for new_move in new_moves: # For move 2
                            score = -testMove(new_space, new_move, simboard)

                            if score < new_score:
                                new_score = score

                            if base_score + new_score < best_score:
                                flag = True
                                break
    ...
    return best_move[1], best_move[0], board
