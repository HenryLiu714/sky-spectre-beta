# Note: Requires an external script to generate moves
...
from performmove import doMove
...
def PMoves(pos, board): # Returns all possible moves for a pawn, bit sketch, only one that also return capture squares (excluding enpassants)
    ...
    if piece.team:
        ...
        if pos-8 > 0 and board[pos-8] == ' ':
            possible_moves.append(pos-8)
        if pos-9 >0 and board[pos-9] != ' ':
            if board[pos-9].team == 1: 
                possible_moves.append(pos-9)
            capture_moves.append(pos-9)
        if pos-7 > 0 and board[pos-7] != ' ':
    ...
    while h < len(possible_moves):
        r1, f1 = posToRankFile(possible_moves[h])
        if abs(f1-file) > 1:
            possible_moves.pop(h)
            h -= 1
        h += 1

    while h < len(capture_moves):
        r1, f1 = posToRankFile(capture_moves[h])
        if abs(f1-file) > 1:
            capture_moves.pop(h)
            h -= 1
        h += 1
...
def checkEndGame(board):
    ...
    if inCheckmate(board) == 1:
        bscore += 1
        print('CHECKMATE! BLACK WINS!')
        ingame = False
    elif inCheckmate(board) == 0:
        wscore += 1
...
def doPromotion(board, pos):
    if board[pos].team == player:
        ...
        if player == 0:
            pieces = [wq, wr, wb, wn]
            board[pos] = random.choice(pieces)
        else:
            pieces = [bq, br, bb, bn]
            board[pos] = random.choice(pieces)
                ...
                if event.type == pg.MOUSEBUTTONUP and not promotion:
                    ...
                    if piece_clicked:
                        ...
                        if space in possible_moves:
                            ...
                            board2 = fenToBoard(state)
                            state1 = getFEN(turn, castle, enpassant_target, halfmove_clock, fullmove_num)
